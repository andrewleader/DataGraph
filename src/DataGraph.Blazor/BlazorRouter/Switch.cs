...
namespace BlazorRouter
{
    public class Switch : ComponentBase, IDisposable
    {
        ...
        {
            var path = UriHelper.ToBaseRelativePath(this.location);
        ...
        {
            this.baseUri = UriHelper.BaseUri;
            this.location = UriHelper.Uri;
            UriHelper.LocationChanged += LocationChanged;
            return Task.CompletedTask;
        }

        public Task RegisterRoute(RenderFragment fragment, string template, bool matchChildren)
        {
            routes.Add(template, matchChildren, fragment);
            return Task.CompletedTask;
        }
